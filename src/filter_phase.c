...
static void* filter_thread(void *args){
    ...
    while (1){
        ...
        /*
        if(CHECK_CHUNK(c,CHUNK_SEGMENT_FLAG)){
            update_flag = 1; // 不需要更新键值存储
        }*/
        free_chunk(c);
        ...
        for(; iter != end; iter = g_sequence_iter_next(iter)){
            ...
            /* A cfl-switch for rewriting out-of-order chunks. 没开*/
            ...
            if (!CHECK_CHUNK(c, CHUNK_DUPLICATE)
                ...
                if(container_overflow(storage_buffer.container_buffer,c->size)){
                    ...
                    write_container_async(storage_buffer.container_buffer);//将容器写入到磁盘
                    TIMER_BEGIN(1);
                    storage_buffer.container_buffer = create_container();
                }
                ...
                if(add_chunk_to_container(storage_buffer.container_buffer,c)){
                    ...
                    if(!CHECK_CHUNK(c,CHUNK_DUPLICATE)){
                        ...
                        g_hash_table_insert(recently_unique_chunks,&wc->fp,wc); //insert unique chunk into recently_unique_chunks table.
            ...
            assert(c->id != TEMPORARY_ID); //此时块已经有容器存储
        ...
        for(; iter != end; iter = g_sequence_iter_next(iter)) {
            ...
            if(r==NULL){
                ...
                r = new_file_recipe_meta(c->data);//file recipe
        ...
        if(destor.index_category[1] == INDEX_CATEGORY_LOGICAL_LOCALITY){
            ...
            if(destor.index_category[0] == INDEX_CATEGORY_EXACT){
                 ...
                 * unique fingerprint are inserted.
                 */
                NOTICE("Filter phase: add %d unique fingerprints to %d features.",
                       g_hash_table_size(recently_unique_chunks),
...
void start_filter_phase(){
    ...
    printf("Filter Phase:start filter pipeline\n");
}
...
//    sync_queue_free()
    close_har();
    printf("Filter Phase: stop filter pipeline successfully!\n");
}