...
void load_config_from_string(sds config) {
	...
	for (i = 0; i < totlines; i++) {
		...
		}else if(strcasecmp(argv[0], "segment-update-similarity-rate") == 0
                 && argc == 2){
            destor.update_rate = atoi(argv[1]);
...
void load_config() {
	...
	FILE *fp;
	if ((fp = fopen("server.config", "r")) == 0) {
		destor_log(DESTOR_WARNING, "No server.config file!");
		return;
	}
