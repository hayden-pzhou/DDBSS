...
GHashTable* (*sampling_all)(GSequence *chunks, int32_t chunk_num);
...
static GHashTable* index_sampling_random(GSequence *chunks, int32_t chunk_num) {
    //
//    NOTICE("sampling phase: random sampling");
    ...
    for (; iter != end; iter = g_sequence_iter_next(iter)) {
        ...
        if ((*head) % destor.index_sampling_method[1] == 0) { //后四个字节%抽样率 == 0
            if (!g_hash_table_contains(features, &c->fp)) { //段内非重复
    ...
    if (g_hash_table_size(features) == 0) {
        /* No feature? add first chunk fp as feature*/
        WARNING("Dedup phase: no features are sampled");
        assert(chunk_num == 0);
        char *new_feature = malloc(destor.index_key_size);
        struct chunk* c = g_sequence_get(g_sequence_get_begin_iter(chunks));
        memcpy(new_feature, &c->fp, destor.index_key_size);
        g_hash_table_insert(features, new_feature, NULL);
    }
    return features;
...
//    NOTICE("Sampling method uniform");
    assert(destor.index_sampling_method[1] != 0);
    GHashTable * features = g_hash_table_new_full(g_feature_hash,
            g_feature_equal, free, NULL);
    int count = 0;
...
static GHashTable* index_sampling_all(GSequence *chunks, int32_t chunk_num) {
//    NOTICE("Sampling method sampling all");
    GHashTable * features = g_hash_table_new_full(g_feature_hash,
                                                  g_feature_equal, free, NULL);
    ...
    if (g_hash_table_size(features) == 0) {
        /* No feature? Empty segment.*/
        assert(chunk_num == 0);
        NOTICE("Dedup phase: An empty segment and thus no feature is selected!");
        char *new_feature = malloc(destor.index_key_size);
        memset(new_feature, 0x00, destor.index_key_size);
        g_hash_table_insert(features, new_feature, NULL);
    }
...
void init_sampling_method(){
    sampling_all = index_sampling_all;
