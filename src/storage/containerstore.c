...
void init_container_store() {
	...
	container_buffer = sync_queue_new(25); //25个容器队列
...
void write_container(struct container* c) {
	...
	if (destor.simulation_level < SIMULATION_APPEND) {
        ...
        //
		unsigned char * cur = &c->data[CONTAINER_SIZE - CONTAINER_META_SIZE]; //数据区
		ser_declare;
		...
		while (g_hash_table_iter_next(&iter, &key, &value)) {
			...
			ser_bytes(&me->fp, sizeof(fingerprint));//copy
...
int add_chunk_to_container(struct container* c, struct chunk* ck) {
	...
	if (destor.simulation_level < SIMULATION_APPEND)
		memcpy(c->data + c->meta.data_size, ck->data, ck->size);//复制数据
